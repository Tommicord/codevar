...
#[cfg(test)]
mod tests {
    use super::{Error, InvalidCodePoint, InvalidControl, InvalidToken, LengthMismatch};

    #[test]
    fn constructors_and_display() {
        assert_eq!(
            Error::length_mismatch(4, 2),
            Error::LengthMismatch(LengthMismatch::new(4, 2))
        );
        assert_eq!(
            Error::invalid_token(9),
            Error::InvalidToken(InvalidToken(9))
        );
        assert_eq!(
            Error::invalid_control(8),
            Error::InvalidControl(InvalidControl(8))
        );
        let _ = format!("{}", Error::InvalidFrame);
        let _ = format!("{}", Error::TruncatedFrame);
        let _ = format!("{}", Error::InvalidMatch);
        let _ = format!("{}", Error::InputTooLarge);
        let _ = format!("{}", Error::InvalidIndex);
        let _ = format!("{}", Error::InvalidDuplicateIndex);
        let _ = format!("{}", Error::OutputTooSmall);
        let _ = format!("{}", Error::InvalidCodePoint(InvalidCodePoint(0xff)));
        let _ = format!("{}", Error::length_mismatch(1, 0));
        let _ = format!("{}", Error::invalid_token(1));
        let _ = format!("{}", Error::invalid_control(2));
    }

    #[test]
    fn from_impls() {
        let _: Error = InvalidToken(1).into();
        let _: Error = LengthMismatch::new(1, 2).into();
        let _: Error = InvalidCodePoint(3).into();
        let _: Error = InvalidControl(4).into();
    }
}
