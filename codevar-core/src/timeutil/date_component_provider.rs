...
use crate::timeutil::date_format_description_modifier::DayPeriod;
use crate::timeutil::date_formatting::fmt_types::{
...
use crate::timeutil::date_time::{
    Hours, Minutes, Nanoseconds, Seconds, Subseconds, Time,
};
use crate::timeutil::date_timestamp::Timestamp;
...
macro_rules! component_provider_methods {
    ($(
        $(#[$meta:meta])*
...
pub(crate) trait ComponentProvider {
    ...
    component_provider_methods! {
        ...
        ("time") period => DayPeriod;
        /// Obtain the second within the minute.
        ("time") second => Seconds;
        /// Obtain the second within the minute.
        ("time") subsecond => Subseconds;
...
impl ComponentProvider for Time {
    ...
    fn hour(&self, _: &mut Self::State) -> Hours {
        Hours::from(self.as_hms_nano_ranged().0)
    }

    #[inline]
    fn minute(&self, _: &mut Self::State) -> Minutes {
        Minutes::from(self.as_hms_nano_ranged().1)
    }

    #[inline]
    fn period(&self, _: &mut Self::State) -> DayPeriod {
        if (*self).hour() < 12 {
            DayPeriod::Am
        } else {
            DayPeriod::Pm
    ...
    fn second(&self, _: &mut Self::State) -> Seconds {
        Seconds::from(self.as_hms_nano_ranged().2)
    }
...
impl ComponentProvider for Timestamp {
    ...
    #[inline]
    fn period(&self, state: &mut Self::State) -> DayPeriod {
        let time = state.time.get_or_insert_with(|| self.time());
