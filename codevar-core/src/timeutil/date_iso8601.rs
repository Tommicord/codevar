...
use crate::timeutil::date_error::Error;
use crate::timeutil::date_format_description_modifier::Padding;
...
use crate::timeutil::date_unit::{Hour, Minute, Nanosecond, Second};
    ...
    V: ComponentProvider,
{
    ...
    match Iso8601::<CONFIG>::TIME_PRECISION {
        ...
        TimePrecision::Minute { decimal_digits } => {
            bytes += format_two_digits(
                output,
                <ru8<0, 99>>::from(value.hour(state).expand()),
                Padding::Zero,
            )?;
            bytes += write_if(output, Iso8601::<CONFIG>::USE_SEPARATORS, ":")?;
        ...
        TimePrecision::Second { decimal_digits } => {
            ...
            bytes += format_two_digits(
                output,
                <ru8<0, 99>>::from(value.minute(state).expand()),
                Padding::Zero,
            )?;
            bytes += write_if(output, Iso8601::<CONFIG>::USE_SEPARATORS, ":")?;
            bytes += format_two_digits(
                output,
                <ru8<0, 99>>::from(value.second(state).expand()),
                Padding::Zero,
            )?;
