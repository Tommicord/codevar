...
mod tests {
    ...
    fn write_app_fragments_at_max_fragment_length() {
        ...
        first
            .payload
            .extend_from_slice(&second.payload);
    ...
    fn application_data_round_trip_through_ten_thousand_messages() {
        ...
        rx.record
            .feed_ciphertext(&wire);
        let mut count = 0usize;
        while let Some(rec) = rx
            .record
            .read_raw()
            .unwrap()
        {
    ...
    fn send_handshake_raw_fragments_and_updates_transcript() {
        ...
        let transcript_before = state
            .transcript
            .bytes()
            .len();
        state
            .send_handshake_raw(&big)
            .unwrap();
        assert_eq!(
            state
                .transcript
    ...
    fn io_state_reports_pending_interest() {
        let mut state = CommonState::new();
        let io = IoState {
            plaintext_bytes_to_read: state
                .app_rx
                .len(),
            tls_bytes_to_write: state
                .record
                .pending_tx_len(),
