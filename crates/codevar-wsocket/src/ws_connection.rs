...
impl CommonState {
    ...
    fn handle_frame(&mut self, frame: WsFrame) -> WsResult<()> {
            ...
            .opcode
        {
            ...
            WsOpcode::Pong => {
                self.messages
                    .push_back(WsMessage::Pong(frame.payload));
            }
    ...
    fn deliver_complete(&mut self, opcode: WsOpcode, payload: Vec<u8>) -> WsResult<()> {
        ...
        match opcode {
            ...
            WsOpcode::Binary => {
                self.messages
                    .push_back(WsMessage::Binary(payload));
            }
            _ => {
...
mod tests {
    ...
    fn broken_utf8_in_fragments_and_single_frames_fails() {
        ...
        let err = s
            .process_frames()
            .expect_err("broken continuation");
    ...
    fn local_close_then_peer_close_completes_handshake() {
        let mut s = open_client();
        s.close(WsCloseCode::GoingAway, "bye")
            .expect("close");
    ...
    fn reserved_opcodes_are_rejected() {
        ...
        let err = s
            .process_frames()
            .expect_err("reserved data opcode");
        assert!(matches!(err, WsError::Protocol { .. }));
